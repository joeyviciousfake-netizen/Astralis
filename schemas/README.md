...
Status: V1 — 8 schemas + conteúdo oficial FM importado (`examples/`) + pack de importação (`packs/`).
...
schemas/
  ...
  card_layout.schema.json  <- 1 molde da carta em DADO, V1 só monstro (doc 04.8)
...
14. Campo de Testes (COMPATÍVEL, `schema_version` continua 1 — só adição opcional em `duel_setup.schema.json`, dado FM existente sem `test_state` continua válido):
     - `test_state` opcional: `my_hand` 0-5 Card IDs (o resto o runtime completa até 5, doc 13.3); `p0_monster/p0_spell/p1_monster/p1_spell` cada um com exatos 5 slots (`null` = vazio, ou `{card_id + face_up? + attack_position?}` — ausentes = `true`, face p/ cima em Ataque, padrão D24; o runtime mapeia p/ `face_down/position` dele, sem mecânica nova).
     - Tudo opcional; IDs estáveis `snake_case` referenciando cartas (gate R4 no Studio: fora da lista = erro, projeto sem carta = aviso); slots seguem o padrão `p0/p1 m/s 0-4` (doc 13.10); LP/seed/ordem NÃO se duplicam — valem `starting_lp/seed/turn_order` do topo.
      - Quando presente, `turn_order` tem que ser `first_p1` (schema `allOf/if-then` + Studio `checar_test_state`): o teste sempre começa na SUA fase da mão (doc 13.3 + D24) — documentado no contrato, executado no runtime (R1/R2/R4).
      - Mínimo válido: `{"schema_version":1,"duel_id":"duel_teste","duelist1":{"duelist_id":"duelist_hero","deck_id":"deck_a"},"duelist2":{"duelist_id":"duelist_rival","deck_id":"deck_b"},"starting_lp":4000,"turn_order":"first_p1","seed":42,"arena_id":"arena_starter","win":{"on_lp_zero":true,"on_deckout":true},"test_state":{"my_hand":["card_a"],"p0_monster":[{"card_id":"card_a"},null,null,null,null],"p0_spell":[null,null,null,null,null],"p1_monster":[null,null,null,null,null],"p1_spell":[null,null,null,null,null]}}`. Inválidos: `my_hand` com 6 cartas, zona com 4 slots, `turn_order:"random"` com `test_state`, `card_id:"Carta-Invalida"`.
15. Molde da carta em DADO (COMPATÍVEL, `schema_version` 1 — schema novo, nenhum existente tocado; doc 04.8):
...
|---|---|---|
| `python tools/fm_import.py --check` | `card.schema.json` + `deck.schema.json` contra as 722 de `examples/cards/` e o pack inteiro + `card_layout.schema.json` (enum do schema bate no espelho, default oficial de monstro com 9 peças, mínimo válido + 6 inválidos) | **único validador automático do repo** (GAP 3). Sai com `rc=0` e `CHECK OK`. O espelho do card dentro do script ainda só checa `>= 0` em ATK/DEF (limite do `--check` = código do QA, não o schema). Não cobre `duel_setup.test_state` (só garante que a extensão não quebrou carta/deck: 722/39/39/25081 intactos). |
...
| Studio `checar_card_layout` (`main.rs`, 7 testes Rust) | `card_layout.schema.json` (versão/id/nome, `layout_for==monster`, canvas 59x86, 0-9 peças kind único, rect dentro do canvas, style, `visible_when`) | **Espelho da mesma regra do schema.** Valida o default oficial + futuro molde por carta (V2 liga no salvar/validar). |
...
3. **Só 3 dos 8 schemas têm validador automático + espelho no Studio.** `card.schema.json`, `deck.schema.json` e `card_layout.schema.json` são cobertos por `tools/fm_import.py --check` (o do molde valida o default oficial de monstro + mínimo válido/inválido, sem tocar em carta/deck). `duel_setup.test_state` (Campo de Testes) é coberto pelo espelho `checar_test_state` no Studio (`main.rs`) + `validar_projeto` — ausente = neutro, quebrado = erro; `card_layout` tem o espelho `checar_card_layout` + 7 testes Rust (gate pronto p/ a V2 ligar no salvar/validar). `arena.schema.json`, `effect.schema.json` e `fusion.schema.json` **continuam sem validação automática em lugar nenhum** — nem pelo `--check`, nem pelo `runtime_validator.gd`, nem pelo Studio (que valida LP e `flow.mode` no seu próprio código Rust, não pelo schema). Consequência restante: um `arena.json` com x/y fora da tela só é pego na hora de desenhar. O `test scenario` completo (GIVEN/WHEN/THEN do doc 10.3) segue sem schema — o `test_state` fecha só o context de campo+mão (doc 10.2).
...
- Runtime (astralis/): carrega estes arquivos; preview/test usam os mesmos dados (R1/R2). Nenhuma mudança nesta sessão afeta o runtime: os limites novos só passam a valer onde o JSON é validado contra o schema. Arena é só layout: lógica usa ID fixo, desenho usa x/y com fallback p/ grade padrão (D24). Mão é só layout: se `hand` faltar, runtime usa fallback (p0 embaixo, p1 em cima); `p1` sempre desenha de costas. Campos FM novos (`guardian_star_*`, `password`, `starchip_cost`, `equip`, `ritual`) são DADO aceito e ignorado na mesa V1 — sem regra nova, sem Fake (R1/R4 ok). `test_state` é DADO aceito e ainda ignorado na mesa V1: o runtime vai ler depois (ver retorno ao Lead); até lá, o duelo com `test_state` sobe como duelo normal.
- Editor (Studio): o Studio já exigia 0..9999 em ATK/DEF (`main.rs:427/435`), `sequence` em flow (`main.rs:1199`) e ≤ 99999 em vida (`main.rs:1303`). Os schemas passaram a dizer o mesmo — **nada muda para o editor, que agora está alinhado ao contrato**. Ele **não** valida x/y de slot: um slot fora de 1920x1080 passa no editor e no schema, e só aparece errado na mesa. Novo nesta sessão: `checar_test_state` (espelho de `duel_setup.test_state`) + gancho em `validar_projeto` (Duelo) + 6 testes Rust — o Campo de Testes valida mão/campo/ordem em PT-BR sem calcular jogo.
- Compatibilidade: os 4 apertos são **compatíveis** (só added bounds/enum já existente) — `schema_version` continua **1**, nenhuma mudança de `required`, nenhum enum existente alterado, nenhum arquivo de dado tocado (doc 04.3). A extensão `test_state` é **compatível** pelo mesmo motivo: campo novo opcional, `required` intacto, `allOf/if-then` só dispara quando o campo novo está presente (dado FM sem `test_state` nem passa pelo `then`). O `card_layout.schema.json` é **novo e compatível** pelo mesmo motivo: nenhum schema existente foi tocado, `schema_version` continua **1**, sem campo novo na carta (molde por carta = V2, doc 04.8).
