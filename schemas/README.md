...
3. **Só 2 dos 7 schemas têm validador automático + espelho no Studio p/ o teste.** `card.schema.json` e `deck.schema.json` são cobertos por `tools/fm_import.py --check`. `duel_setup.test_state` (Campo de Testes) é coberto pelo espelho `checar_test_state` no Studio (`main.rs`) + `validar_projeto` — ausente = neutro, quebrado = erro. `arena.schema.json`, `effect.schema.json` e `fusion.schema.json` **continuam sem validação automática em lugar nenhum** — nem pelo `--check`, nem pelo `runtime_validator.gd`, nem pelo Studio (que valida LP e `flow.mode` no seu próprio código Rust, não pelo schema). Consequência restante: um `arena.json` com x/y fora da tela só é pego na hora de desenhar. O `test scenario` completo (GIVEN/WHEN/THEN do doc 10.3) segue sem schema — o `test_state` fecha só o context de campo+mão (doc 10.2).
